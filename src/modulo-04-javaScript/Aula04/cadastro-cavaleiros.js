...
$(document).ready(function(){
  ...
  (function carregaImg(indice) {
    ...
    /*$('#cavaleiros img').mouseclick(function() {
    var self = $(this);
    var cavaleiro = goldSaints[self[0].id-1];
    for(var campo in cavaleiro) {
      $('#detalhes-cavaleiro').append($('<strong>').text(campo+ ': ')).append(cavaleiro[campo]).append($('<br>'));
      }
    });
​
     $('#imagens img').mouseout(function() {
     $('#detalhes-cavaleiro').empty();
    });*/
      
      
      $img.on('click mouseclick', function() {
        var self = $(this);
        var nome = self.attr('alt');
        var altura = goldSaints.filter(function(elem) {
          return elem.id === parseInt(self.attr('id'));
        })[0].alturaCm;
        var peso = goldSaints.filter(function(elem){
          return elem.id === parseInt(self.attr('id')); 
        })[0].pesoLb;
        ...
        $detalhesCavaleiro.append($('<h3>').text(Math.round(peso*100)/100));
      });
      /*setTimeout(function() {
        $img.off('mouseleave');
      }, 5000);*/
...
var contgolpe=0;
$('#adiconar-golpe').click(function(){
  contgolpe++;
  var $input = $('<input>').attr('id','id-golpes').attr('placeholder','Golpe ' + contgolpe);
  $("#listaGolpe").append($('<span>').append($input));
});

var contImg=0;
$('#adicionar-Img').click(function(){
  contImg++;
  var $input = $('<input>').attr('id','id-imagem').attr('placeholder','Imagem ' + contImg);
  $("#listaImagem").append($('<span>').append($input));
});