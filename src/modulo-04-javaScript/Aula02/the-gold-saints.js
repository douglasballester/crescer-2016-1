...
//Funções para diminuir repetição
Number.prototype.converteEmMetros = function(){
  return this / 100;
};
...
var converteEmKilos = function(pesoEmLibras){
  return pesoEmLibras * 0.4546;
};

//Exercicio 01
/*function obterDoadores(){
    return goldSaints.filter(
      function(e){
...
function obterCavaleiroComMaisGolpes(){
  var cavMaisGolpes;
    for(var i = 0; i < goldSaints.length; i++){
      if(i === 0){
        cavMaisGolpes = goldSaints[i];
      }
      else if(cavMaisGolpes.golpes.length < goldSaints[i].golpes.length){
        cavMaisGolpes = goldSaints[i];
      }
    }
    return cavMaisGolpes;
}
...
function obterAlturaMedia(){
  var somaAltura = 0;
  for (var i = 0; i < goldSaints.length; i++) {
    somaAltura += goldSaints[i].alturaCm;
  }
  somaAltura = (somaAltura / 12);
  return formatarNumero(somaAltura.converteEmMetros());
}
...
function obterAlturaMediana (){
   ...
   for(var i = 0; i < goldSaints.length; i++){
     arrayDasAlturas.push(goldSaints[i].alturaCm);
   }
   arrayDasAlturas.sort(sortfunction);
   mediana = ((arrayDasAlturas[5] + arrayDasAlturas[6])/2);
   return formatarNumero(mediana.converteEmMetros());
}
//by StackOverflow
...
function obterPesoMedio(knights){

    var calculaPesoMedio = 0, contCav = 0;
    for (var i = 0; i < knights.length; i++){
      if(knights[i].pesoLb !== undefined){
        calculaPesoMedio += knights[i].pesoLb;
        contCav++;
      }
    }
    calculaPesoMedio = (calculaPesoMedio / contCav);
    return formatarNumero(converteEmKilos(calculaPesoMedio));
}
...
//Exercicio 07
function obterIMC (fn){
  return goldSaints.map(function(e){
      return calcularIMC(e.pesoLb * 0.45, e.alturaCm/100);
  }).filter(Number);
}
...
function obterSobrepeso() {
    return goldSaints.filter(function(e){
        var imc = calcularIMC(e.pesoLb * 0.45,e.alturaCm / 100);
